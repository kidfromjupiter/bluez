...
Connects all LE profiles the remote device supports that can be connected and
...
Disconnects all connected profiles and then terminates the low-level ACL-LE
connection.

ACL-LE connection will be terminated even if some profiles were not
...
For non-trusted devices connected over LE, calling this method will disable
...
| Paired | `boolean` | `readonly`, `experimental` | Indicates if the remote device is paired to the LE bearer. Pairing means devices exchange information to establish an encrypted connection. |
| Bonded | `boolean` | `readonly`, `experimental` | Indicates if the remote device is bonded to the LE bearer. Bonded means pairing information has been stored and will be persisted. |
| Connected | `boolean` | `readonly`, `experimental` | Indicates if the remote device is currently connected to the LE bearer. PropertiesChanged signals indicate changes to this status. |
